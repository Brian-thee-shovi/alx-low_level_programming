 ...
 * Description: magic numbers which are seperated by spaces
 */
void print_magic(unsigned char *e_ident)
{
	int max;
	...
	for (max = 0; max < EI_NIDENT; max++)
	{
		printf("%02x", e_ident[max]);
 ...
 * @e_ident: pointer to an array which contains the ELF class
 */
void print_class(unsigned char *e_ident)
 ...
 * @e_ident: pointer to an array which contains the ELF version
 */
void print_version(unsigned char *e_ident)
{
	 printf(" Version: %d",
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	h = read(p, header, sizeof(Elf64_Ehdr));
	if (h == -1)
	{
		free(header);
		close_elf(p);
